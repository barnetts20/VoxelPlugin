...
struct VOXELPLUGIN_API FMeshStreamData
{
    ...
    FMeshStreamData()
    {
        ...
        MeshStream.AddStream(
            FRealtimeMeshStreams::PolyGroups,
    ...
    TRealtimeMeshStreamBuilder<FVector, FVector3f>
        GetPositionStream()
    {
        return TRealtimeMeshStreamBuilder<FVector, FVector3f>(
            *MeshStream.Find(FRealtimeMeshStreams::Position));
    }


    TRealtimeMeshStreamBuilder<
        FRealtimeMeshTangentsHighPrecision,
        FRealtimeMeshTangentsNormalPrecision>
        GetTangentStream()
    {
        return TRealtimeMeshStreamBuilder<
            FRealtimeMeshTangentsHighPrecision,
            FRealtimeMeshTangentsNormalPrecision>(
                *MeshStream.Find(FRealtimeMeshStreams::Tangents));
    }


    TRealtimeMeshStreamBuilder<TIndex3<uint32>>
        GetTriangleStream()
    {
        return TRealtimeMeshStreamBuilder<TIndex3<uint32>>(
            *MeshStream.Find(FRealtimeMeshStreams::Triangles));
    }
    ...
    TRealtimeMeshStreamBuilder<uint32, uint16>
        GetPolygroupStream()
    {
        return TRealtimeMeshStreamBuilder<uint32, uint16>(
            *MeshStream.Find(FRealtimeMeshStreams::PolyGroups));
    }
        ...
        GetTexCoordStream()
    {
        return TRealtimeMeshStreamBuilder<
            FVector2f,
            FVector2DHalf>(
                *MeshStream.Find(FRealtimeMeshStreams::TexCoords));
    }
    ...
    TRealtimeMeshStreamBuilder<FColor>
        GetColorStream()
    {
        return TRealtimeMeshStreamBuilder<FColor>(
            *MeshStream.Find(FRealtimeMeshStreams::Color));
    }
...
struct VOXELPLUGIN_API FMeshChunk
{
    ...
    FRealtimeMeshLODKey LODKey =
        FRealtimeMeshLODKey::FRealtimeMeshLODKey(0);

    FRealtimeMeshSectionConfig SecConfig =
        ...
        double InExtent)
    {
        ...
        SurfaceMeshData->MeshGroupKey =
            FRealtimeMeshSectionGroupKey::Create(
                LODKey,
                FName("SURFACE"));

        SurfaceMeshData->MeshSectionKey =
            FRealtimeMeshSectionKey::CreateForPolyGroup(
                SurfaceMeshData->MeshGroupKey,
                0);


        OceanMeshData->MeshGroupKey =
            FRealtimeMeshSectionGroupKey::Create(
                LODKey,
                FName("OCEAN"));

        OceanMeshData->MeshSectionKey =
            FRealtimeMeshSectionKey::CreateForPolyGroup(
                OceanMeshData->MeshGroupKey,
                0);
    }
        ...
        UMaterialInterface* InOceanMaterial)
    {
        ...
        ChunkRtMesh =
            NewObject<URealtimeMeshSimple>(InParentActor);

        ChunkRtMesh->SetCollisionConfig(cConfig);

        ChunkRtMesh->SetupMaterialSlot(
            0,
            "Surface Material");
        ...
        ChunkRtComponent =
            NewObject<URealtimeMeshComponent>(
                InParentActor,
        ...
        ChunkRtComponent->AttachToComponent(
            InParentActor->GetRootComponent(),
            FAttachmentTransformRules::KeepRelativeTransform);

        ChunkRtComponent->SetRelativeLocation(
            ChunkCenter);

        ChunkRtComponent->SetCollisionEnabled(
            ECollisionEnabled::QueryAndPhysics);

        ChunkRtComponent->SetCollisionResponseToAllChannels(
            ECollisionResponse::ECR_Block);

        ChunkRtComponent->SetRealtimeMesh(
            ChunkRtMesh.Get());
        ...
        ChunkRtMesh->CreateSectionGroup(
            SurfaceMeshData->MeshGroupKey,
        ...
        TSharedPtr<FMeshChunk> Self)
    {
        AsyncTask(
            ...
            [Self]()
            {
                ...
                {
                    ARealtimeMeshActor* Parent =
                        Self->CachedParentActor.Get();

                    UMaterialInterface* SurfaceMaterial =
                        Self->CachedSurfaceMaterial.Get();

                    UMaterialInterface* OceanMaterial =
                        Self->CachedOceanMaterial.Get();
                ...
                URealtimeMeshSimple* MeshPtr =
                    Self->ChunkRtMesh.Get();

                URealtimeMeshComponent* CompPtr =
                    Self->ChunkRtComponent.Get();
                ...
                auto* SrfTriStream =
                    Self->SurfaceMeshData->MeshStream.Find(
                        FRealtimeMeshStreams::Triangles);

                int32 SrfNumTris =
                    SrfTriStream ? SrfTriStream->Num() : 0;


                if (SrfNumTris <= 0)
                {
                    MeshPtr->UpdateSectionGroup(
                        Self->SurfaceMeshData->MeshGroupKey,
                        FRealtimeMeshStreamSet());
                }
                else
                {
                    MeshPtr->UpdateSectionGroup(
                        Self->SurfaceMeshData->MeshGroupKey,
                        Self->SurfaceMeshData->MeshStream);

                    FRealtimeMeshSectionConfig SrfConfig(0);

                    MeshPtr->UpdateSectionConfig(
                        Self->SurfaceMeshData->MeshSectionKey,
                        SrfConfig,
                        true);
                }
                ...
                auto* OcnTriStream =
                    Self->OceanMeshData->MeshStream.Find(
                        FRealtimeMeshStreams::Triangles);

                int32 OcnNumTris =
                    OcnTriStream ? OcnTriStream->Num() : 0;


                if (OcnNumTris <= 0)
                {
                    MeshPtr->UpdateSectionGroup(
                        Self->OceanMeshData->MeshGroupKey,
                        FRealtimeMeshStreamSet());
                }
                else
                {
                    MeshPtr->UpdateSectionGroup(
                        Self->OceanMeshData->MeshGroupKey,
                        Self->OceanMeshData->MeshStream);

                    FRealtimeMeshSectionConfig OcnConfig(1);

                    OcnConfig.bIsVisible = true;
                    OcnConfig.bCastsShadow = false;

                    MeshPtr->UpdateSectionConfig(
                        Self->OceanMeshData->MeshSectionKey,
                        OcnConfig,
                        false);
                }
